...
1. Скачайте архив с [официального релиза zapret-discord-youtube](https://github.com/Flowseal/zapret-discord-youtube/releases/latest) и распакуйте куда-нибудь без кириллицы и пробелов в пути (например, `C:\zapret-discord-youtube`).
2. Скачайте `ZapretGUI-Setup.exe` со [страницы релизов](https://github.com/sand0o/ZapretGUI/releases) или соберите сами (см. ниже).
3. Запустите ZapretGUI **от имени администратора** (драйвер WinDivert требует прав админа). При первом запуске:
   - Откройте «Настройки» → «Папка zapret-discord-youtube» → выберите папку из шага 1.
   - Выберите стратегию (по умолчанию `general`).
   - Если играете в онлайн-игры — включите режим «Игровой фильтр».
4. Закройте настройки и нажмите большую кнопку.
